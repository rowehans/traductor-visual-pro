...
| `main.spec` | PyInstaller spec para compilar `main.exe` con `console=False` (sin ventana CMD). **Incluye carpeta `js/`** (5 módulos ES importados por `app.js`: config.js, utils.js, toast.js, theme.js, filters.js). CTD eliminado del bundle. **Excluye módulos pesados** (torch, transformers, ct2, easyocr) — se cargan desde `env/` en runtime. **UPX deshabilitado**. .exe resultante: **360MB** (200MB sin RapidOCR, +160MB por modelos ONNX). | 3KB |
